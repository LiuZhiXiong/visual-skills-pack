...
## 🎨 拒绝平庸：前端大师级反 AI 美学 (Anti-Slop Aesthetics)
你必须彻底贯彻顶级 `@frontend-design` 工作流的设计魂魄，杜绝一切“AI 塑料感（通用排版、居中堆叠、老土紫色渐变）”：
1. **意图先行的极端美学 (Intentionality)**：不要做四平八稳的平庸海报。要么选择带刺的“极繁主义 (Maximalist)”，要么选择极其克制的“留白极简 (Minimal)”，让每一次视觉输出都有强烈的性格爆点。
2. **空间与排版破局 (Grid-Breaking)**：必须抛弃陈旧的区块居中对齐！大胆使用**不对称悬浮构图 (Asymmetry)**、**元素的刻意堆叠穿插 (Overlap)** 或 **对角线动势流 (Diagonal Flow)** 来打破沉闷感。
3. **极品字体组合 (Distinctive Typography)**：永远避开烂大街的默认字体（像干瘪的 Arial / System Sans）。为大标题选择极具张力的展示型字体（如超细/超粗加长），为正文搭配精致易读的次级字形，以此拉开震撼的排版错差！
4. **环境光与深度材质 (Textures & Depth)**：坚决抵制光秃秃的单色块背景！强行注入**高级颗粒噪点 (Noise grain)**、**渐变网格底纹 (Gradient Meshes)**、带有折射的**毛玻璃层 (Layered Transparencies)** 以及带物理级光衰的动态环境光，赋予平面的海报“电影级”的视觉厚度！

---

## 📍 核心神工级 CSS 架构指引 (Global Design Architecture)
只要是你写出的 HTML，都**必须强制套用以下底层排版原理**。这不仅能让哪怕最随机的主题都能具备世界级的视觉表现力，还能防止产生廉价人工 AI 味。
