...
**【出图强提醒】**：落盘 `.html` 文件后，你必须调用伴随本技能提供的 `scripts/export.js` 脚本把 HTML 截图返回给用户，**否则用户看不到图，这就等于任务失败！**

脚本位置在与本 `SKILL.md` 平级的 `scripts/export.js`。请动态使用你当前读取本配置的绝对路径来执行它：
```bash
# 如果是电脑宽屏 16:9，你的命令行必须加上宽1920 高1080参数！
node <本SKILL.md所在绝对路径目录>/scripts/export.js <生成的.html文件的绝对路径> 1920 1080

# 如果是小红书正方形 1:1，则这么穿！
node <本SKILL.md所在绝对路径目录>/scripts/export.js <生成的.html文件的绝对路径> 1080 1080

# 默认竖屏画报则只需要！
node <本SKILL.md所在绝对路径目录>/scripts/export.js <生成的.html文件的绝对路径> 1080 1920
```
