...
拿到数据后，从你背后这 **13 套殿堂级预设美学引擎**中，挑选一个最能够震慑人心的方向，并且**强制使用工具 `view_file` 读取对应的指南文件**：

### ☀️ 新晋主打：商业明亮与高能活力 (Bright & Light Corporate)
（如果用户没有特殊要求，特别是像**保险、理财、科技产品对比**这种商业场景，**优先从这里挑选明亮干净的风格**）
- 💳 `./infographic/reference/style-stripe-fintech.md` —— 【Stripe 金融科技明亮风】：纯白绝净画布、超丝滑弥散阴影、少量极高纯度活力色彩。适合高端金融、保险对比、SaaS 官网。
...
2. **暴力指数级字体阶梯 (Typographic Scale)**
   抛弃“凭感觉定字号”。主视觉大标题必须大到带有攻击性：
   - 标题必须具备**极其震撼的比例**，强制使用极客钳制 `font-size: clamp(3rem, 8vw, 8rem);` 配合 `line-height: 0.85; letter-spacing: -0.04em;` 强行挤压。
   - 二级标题不要超过 `1.5rem`。
   - 所有普通正文字段必须降回 `0.875rem (14px)` 或最多 `1rem`，并配以极佳阅读性的 `line-height: 1.6` 以及弱化颜色的 `--text-muted`，拉开百倍的主次错觉差。
   - **安全字体底座**：为了防止 Google Fonts 获取不到，所有 `font-family` 必须垫上终极回退链（如 `system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto...`）。
...
1. **画幅智能感知 (Ratio Detection)**：
   你要根据用户的要求或者内容长短推断需要什么**长宽比例**，以此在顶级容器上设定：
   - 如果是 **电脑宽屏/横版 PPT (16:9)**：`.container { width: 1920px; min-height: 1080px; aspect-ratio: 16/9; margin: 0 auto; overflow: hidden; }`
   - 如果是 **小红书/Instagram 方图 (1:1)**：`.container { width: 1080px; height: 1080px; aspect-ratio: 1/1; margin: 0 auto; overflow: hidden; }`
   - 如果是默认的 **微信海报长图 (9:16 或自由长度)**：`.container { width: 1080px; min-height: 1920px; margin: 0 auto; overflow: hidden; }`
...
持久化落盘写入当前工作目录：`./infographic/xxx.html`。

## 📍 极清动态快门闭环 (Final Trigger)
落盘后，直接在这个目录下召唤带**画幅参数**的无头截屏 API 完成任务闭环：
```bash
# 如果是电脑宽屏 16:9，你的命令行必须加上宽1920 高1080参数！
node scripts/export.js <生成的你的html文件名.html> 1920 1080

# 如果是小红书正方形 1:1，则这么穿！
node scripts/export.js <生成的你的html文件名.html> 1080 1080

# 默认竖屏画报则只需要！
node scripts/export.js <生成的你的html文件名.html> 1080 1920
```
跑满 2x 倍率强采切图。交付完美终端成品时，请向用户汇报今天使用了什么画幅、什么数学框架以及什么硅谷级纯 CSS 审美！
